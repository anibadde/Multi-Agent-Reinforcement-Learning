...
def mlp_model(input, num_outputs, scope, reuse=False, num_units=256, rnn_cell=tf.contrib.rnn.BasicLSTMCell):
    # This model takes as input an observation and returns values of all actions
    with tf.variable_scope(scope, reuse=reuse):
        out = input
        #out = layers.fully_connected(out, num_outputs=num_units, activation_fn=tf.nn.relu)
        #out = layers.fully_connected(out, num_outputs=num_units, activation_fn=tf.nn.relu)
        out = layers.linear(out, num_outputs=num_units, activation_fn=tf.nn.sigmoid)
        out = layers.linear(out, num_outputs=num_units, activation_fn=tf.nn.sigmoid)
...
def make_env(scenario_name, arglist, benchmark=False):
    ...
    if benchmark:
        env = MultiAgentEnv(world, scenario.reset_world, scenario.reward, scenario.observation, info_callback=scenario.benchmark_data, done_callback=scenario.done)
    else:
    ...
    with U.single_threaded_session():
        ...
        while True:
            ...
            if arglist.benchmark:
                #print("BENCHMARK")
                if episode_num > arglist.benchmark_iters and (done or terminal):
                    value = [np.mean(rew[-arglist.save_rate:]) for rew in agent_rewards]
